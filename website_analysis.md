...
Flow: **Hero → 01 Masalah → 02 Produk → 03 Fitur → Footer**
...
| 01 Masalah (`ProblemSection.tsx`) | Header + badge, three tall gradient cards in a row (stack on mobile) | Good scale, big icons, clear "problem" framing | Description is **hidden until hover** on desktop (see U-01); three near-identical dark cards lack differentiation; cards are non-interactive but look clickable |
...
### 4.2 `SectionBadge`
Numbered compound pill (blue circle + label), reused on landing and About, with a dark variant. Good reuse and a clear wayfinding device. Numbering is per page.
...
|---|---|---|---|---|---|
| U-01 | High | Landing | Problem-card descriptions are invisible until hover on `lg`. The `group-focus-within` fallback can't trigger because nothing inside is focusable; touch laptops/keyboard users never see them | `ProblemSection.tsx:22` | Always show description (or make cards focusable with `tabIndex={0}`) |
...
| U-05 | Med | Landing | Section anchors exist (`#masalah`, `#produk`) but navbar has no in-page links, and there is no `scroll-margin-top` for the fixed navbar, so the hero cue lands the heading close under it (**verify**) | `Navbar.tsx:7-12`, `HeroSection.tsx:39` | Add `scroll-mt-24` to sections; optionally add section links |
...
|---|---|
| < `md` | Single column; hero centred; hamburger navbar (solid on scroll, no island); problem cards stack (`min-h-[24rem]`); accordion full width; footer stacks |
| `md` | Desktop nav links + CTA appear; team grid 2 cols |
| `lg` (1024) | Hero card rounded with margin; island navbar; problem cards in a row; sticky product panel; dashboard 2-column |
...
**Quick wins (≤ 1 day)**
1. Show problem-card descriptions permanently (U-01).
2. Add `scroll-mt-*` to anchored sections (U-05).
